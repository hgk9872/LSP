...
int main(void)
{
    ...
    if (sigaction(SIGUSR1, &sig_act, NULL) != 0) { // SIGUSR1 시그널 액션 변경
        ...
        if (sigprocmask(SIG_SETMASK, &sig_set, NULL) != 0) { // SIGUSR1 블록
            ...
            kill(getpid(), SIGUSR1); // SIGUSR1 시그널 발생
            printf("after kill()\n");
            ssu_check_pending(SIGUSR1, "SIGUSR1"); // 시그널 블록상태인지 확인
            sigemptyset(&sig_set);
