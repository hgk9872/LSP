...
int main(void) {
    ...
    act_int.sa_handler = ssu_signal_handler1; // handler를 통해 시그널 상태 출력
    sigemptyset(&act_int.sa_mask);
    sigaddset(&act_int.sa_mask, SIGQUIT);
    act_quit.sa_flags = 0;

    if (sigaction(SIGINT, &act_int, NULL) < 0) { // SIGINT 시그널 액션 변경
        fprintf(stderr, "sigaction(SIGINT) error\n");
        exit(1);
    }

    act_quit.sa_handler = ssu_signal_handler2; // handler를 통해 시그널 상태 출력
    sigemptyset(&act_quit.sa_mask);
    sigaddset(&act_quit.sa_mask, SIGINT);
    act_int.sa_flags = 0;

    if (sigaction(SIGQUIT, &act_quit, NULL) < 0) { // SIGQUIT 시그널 액션 변경
